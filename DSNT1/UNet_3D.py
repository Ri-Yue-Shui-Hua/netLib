...
class Up(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        # self.trans_conv = nn.ConvTranspose3d(in_channels, in_channels,
        #                                      kernel_size=2, stride=2, dilation=2, padding=1, output_padding=1)
        '''
...
def DSNT_f(h):
    ...
    x = torch.arange(-1, end, 2/(X - 1)).to(h.device)
    y = torch.arange(-1, end, 2/(Y - 1)).to(h.device)
    z = torch.arange(-1, end, 2/(Z - 1)).to(h.device)
...
class UNet_3D(nn.Module):
    ...
    def forward(self, x):
        ...
        px, py, pz = DSNT_f(norm_local)
        return norm_local, px, py, pz
