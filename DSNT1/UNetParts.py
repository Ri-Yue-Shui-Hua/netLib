...
class OutConv(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        nn.init.constant_(self.conv.bias, 0.0005)
        # self.conv = nn.Conv3d(in_channels, out_channels,kernel_size=1)
...
class Spacial_Info(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        nn.init.constant_(self.conv6.bias, 0.0005)
        self.tanh = nn.Softmax()
    ...
    def forward(self, x):
        ...
        x = self.conv5(x)
        x = self.conv6(x)
        #x = torch.exp(x) / torch.exp(x).sum(dim=(2, 3, 4), keepdim=True)
        x = self.tanh(x)
